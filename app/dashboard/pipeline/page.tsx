      ...
      <header className="sticky top-0 z-20 border-b border-gray-200 bg-white/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-screen-2xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            <Link href="/dashboard/legal" className="rounded-lg px-3 py-2 text-sm text-gray-500 transition hover:bg-gray-100">
              Rechtliches
            </Link>
            <LogoutButton />
      ...
      <main className="mx-auto max-w-screen-2xl space-y-5 px-6 py-8">
        ...
        {sites.length === 0 && !fetchError && (
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-white py-28 text-center">
            <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-gray-900 text-2xl font-black text-white">W</div>
            <h2 className="mb-2 text-xl font-bold text-gray-900">Pipeline ist leer</h2>
            <p className="mb-7 max-w-xs text-sm text-gray-500">Erstelle deine erste Demo-Website für einen Lead.</p>
            <Link href="/dashboard/new" className="rounded-xl bg-gray-900 px-7 py-3 text-sm font-semibold text-white transition hover:bg-gray-700">
        ...
        {sites.length > 0 && (
          <>
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              <p className="mb-5 text-[11px] font-semibold uppercase tracking-widest text-gray-400">Sales Pipeline</p>
                ...
                {[
                  ...
                  <div key={stage.name} className="flex flex-1 items-center">
                    <div className="flex-1 text-center">
                      <div className={`text-3xl font-black tabular-nums ${stage.num}`}>{stage.count}</div>
                    ...
                    {i < arr.length - 1 && (
                      <svg className="h-4 w-4 flex-shrink-0 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                    )}
                  </div>
                ))}
              ...
              <div className="flex h-1.5 overflow-hidden rounded-full bg-gray-100 gap-px">
                {byStatus.Entwurf > 0 && <div className="rounded-full bg-gray-300" style={{ width: `${byStatus.Entwurf / sites.length * 100}%` }} />}
                {byStatus.Gesendet > 0 && <div className="rounded-full bg-blue-400" style={{ width: `${byStatus.Gesendet / sites.length * 100}%` }} />}
                {byStatus.Interessiert > 0 && <div className="rounded-full bg-amber-400" style={{ width: `${byStatus.Interessiert / sites.length * 100}%` }} />}
                {byStatus.Gewonnen > 0 && <div className="rounded-full bg-green-500" style={{ width: `${byStatus.Gewonnen / sites.length * 100}%` }} />}
                {byStatus.Abgelehnt > 0 && <div className="rounded-full bg-red-300" style={{ width: `${byStatus.Abgelehnt / sites.length * 100}%` }} />}
              </div>
            </div>
