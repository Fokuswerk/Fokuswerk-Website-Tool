...
import { DEFAULT_AGB } from "@/lib/legalTemplates";
...
async function getData(slug: string) {
  const [siteRes, settingsRes] = await Promise.all([
    ...
    supabase.from("settings").select("value").eq("key", "agb_template").single(),
  ]);
  const site = siteRes.data as Site | null;
  // Per-site AGB takes priority; fall back to global template, then DEFAULT_AGB
  const template = site?.agb_text || settingsRes.data?.value || DEFAULT_AGB;
  ...
  return (
    <div className="min-h-screen bg-white font-sans antialiased">
      ...
      <main className="mx-auto max-w-4xl px-6 py-16">
        <h1 className="mb-10 text-4xl font-bold tracking-tight text-gray-900">Allgemeine Geschäftsbedingungen</h1>
        <div className="prose prose-gray max-w-none">
          {lines.map((line, i) => {
            if (line.startsWith("## ")) return <h2 key={i} className="mt-8 mb-3 text-xl font-bold text-gray-900">{line.slice(3)}</h2>;
            if (line.startsWith("# ")) return null;
            if (line.match(/^### /)) return <h3 key={i} className="mt-6 mb-2 text-lg font-semibold text-gray-800">{line.slice(4)}</h3>;
